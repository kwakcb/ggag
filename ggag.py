...
# Sample Data
    ...
    #data
    data = {
        "bonbu": ["충남&충북NW운용본부(N)","부산&경남NW운용본부(N)","충남&충북NW운용본부(N)","전남&전북NW운용본부(N)","부산&경남NW운용본부(N)",
        ...
        "center": ["충북액세스운용센터","부산액세스운용센터","충남액세스운용센터","전남액세스운용센터","경남액세스운용센터",
        ...
        "ktteam1": ["서청주운용팀","남부산운용팀","서대전운용팀","해남운용팀","거제운용팀",
        ...
        "ktteam2": ["진천운용팀","동부산운용부","대전운용팀","목포운용부","동진주운용부",
    ...
    # 국사입력
    input_kuksa = st.text_input("조회할 국사 입력")

    # IP로 검색
    if input_kuksa:
        result = df[df["office"] == input_kuksa]
        if not result.empty:
            #st.success("조회 결과:")
            st.write(result)
        else:
